    ...
    const fetchAllData = async () => {
      try {
        const [branches, payments] = await Promise.all([
          fetchData(`${API_BASE}/branches`),
          fetchData(`${API_BASE}/payments?pageSize=200`),
        ]);

        const payItems = payments.items || [];
        const branchTotals = {};
        payItems.forEach(p => {
          if (p.branchName) {
            branchTotals[p.branchName] = (branchTotals[p.branchName] || 0) + (p.amount || 0);
          }
        });

        const branchAmounts = branches.map(b => ({
          branchName: b.name || b.branchName,
          totalAmount: branchTotals[b.name] || 0,
        }));
